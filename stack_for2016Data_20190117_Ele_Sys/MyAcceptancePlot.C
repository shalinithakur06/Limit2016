...
double getSigEff(TString fileName, TString histDir, TString mass){
  TH1F * histZTag = (TH1F*) getHistoSig(fileName, "base/Iso", "ZTag", "mlZ_max");
  TH1F * histLCut = (TH1F*) getHistoSig(fileName, "base/Iso", histDir, "mlZ_max_sig"+mass);
  double evtZTag = histZTag->Integral();
  double evtLCut = histLCut->Integral();
  double effSig = 0.0;
  cout<<"evtZTag = "<<evtZTag<<endl;
  effSig = evtLCut/evtZTag;
  return effSig;
}
...
void MyAcceptancePlot(){
  ...
  for(int i = 0; i<massFiles.size(); i++){
    ...
    TH1F * histLCut = (TH1F*) getHistoSig(massFiles[i], "base/Iso", "ZTag3", "mlZ_max_sig"+TString(massLabel[i]));
    double evtLCut = histLCut->Integral();
    double acc = evtLCut/(lumi*sigXss[i]);
    double eff = getSigEff(TString(massFiles[i]), lCutDir, TString(massLabel[i]));
    cout<<"evtLCut = "<<evtLCut<<endl;
    cout<<"eff = "<<eff<<endl;
    cout<<"acc = "<<acc<<endl;
    cout<<"acc*eff = "<<acc*eff<<endl;
    cout<<"xss = "<<sigXss[i]<<endl;
    massVec.push_back(sigMass[i]);
    effVec.push_back(acc*eff);
    }
  can->cd();
  TGraph * myGraph = makeGraph(massVec, effVec);
  if(isMuChannel)decorateGraph(myGraph, "M_{#mu^{*}} [GeV]", "Acceptance #times Efficiency", "", 0, 1, kRed+1);
  if(isEleChannel)decorateGraph(myGraph, "M_{e^{*}} [GeV]", "Acceptance #times Efficiency", "", 0, 1, kRed+1);
